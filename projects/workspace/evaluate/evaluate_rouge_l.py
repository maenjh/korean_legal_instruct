# /workspace/evaluate/evaluate_rouge_l.py  (수정본)
...
def normalize_text(s: str, lower: bool=True, strip_punct: bool=True, collapse_ws: bool=True) -> str:
    if s is None: return ""
    s = s.strip()
    if lower: s = s.lower()
    s = s.replace("\u00A0", " ").replace("\u200b","")
...
def main():
    ...
    ap.add_argument("--char_level", action="store_true", help="문자 단위 토크나이저 사용(한국어 권장)")
    args = ap.parse_args()
    ...
    # ✅ 버전 호환용 문자 토크나이저 (rouge-score에 없을 수 있어서 직접 구현)
    class CharTokenizer:
    ...
    for i,(pr,gr) in enumerate(pairs):
        ...
        s = scorer.score(g, p)['rougeL']   # ref=g, pred=p
