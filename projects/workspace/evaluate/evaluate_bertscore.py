# /workspace/evaluate/evaluate_bertscore.py
"""
BERTScore evaluator for JSONL prediction/gold files.

- Aligns by id if present; otherwise by index.
- Outputs: per-sample CSV + overall summary JSON.
- Defaults assume fields: id, prediction (pred), answer (gold)

Example:
  python3 /workspace/evaluate/evaluate_bertscore.py \
    --pred /workspace/outputs/qlora-sft-civil-law-llama/predictions.jsonl \
    --gold /workspace/data/gold_val.jsonl \
    --out_csv /workspace/evaluate/bertscore_details.csv \
    --out_json /workspace/evaluate/bertscore_summary.json \
...
def main():
    ...
    ap.add_argument("--model", default="xlm-roberta-large", help="Hugging Face model name")
    ap.add_argument("--lang", default="ko", help="Language code for baselines (e.g., ko, en)")
    ...
    if not pairs:
        print("No aligned pairs found. Check id_field and inputs.", file=sys.stderr)
        sys.exit(2)
